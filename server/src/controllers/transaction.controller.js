...
module.exports = {
    ...
    try {
      ...
      if (transaction) {
        if (book.availableCopies < 1) {
          return res
            .status(404)
            .json({ msg: "No copies of this book is left in the store." });
        } else {
          Book.findByIdAndUpdate(
            ...
            .then((updatedBook) =>
              console.log("Updated book after rental:", updatedBook)
            )
  ...
  getAllTransaction: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ count: transactions.length, data: transactions });
    } catch (err) {
      console.log(err.message);
  ...
  getTransactionByTransactionId: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ count: transaction.length, data: transaction });
    } catch (err) {
      console.log(err.message);
      res.status(500).send({ msg: err.message });
    }
  ...
  getTransactionByUserId: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ count: userTransaction.length, data: userTransaction });
    } catch (err) {
      console.log(err.message);
      res.status(500).send({ msg: err.message });
    }
  ...
  bookReturned: async (req, res) => {
    try {
      ...
      const bookDueDate = new Date(
        new Date().setDate(new Date().getDate() + dueDate)
      );
      ...
      if (currentTime > bookDueDate) {
        ...
        const timeDifferenceInDays =
          timeDifferenceInMillis / (1000 * 60 * 60 * 24);
        ...
        return res
          .status(200)
          .json({ msg: "Transaction deleted", fee, lateFee, totalFee });
      } else {
        const fee = book.rentalFee;
